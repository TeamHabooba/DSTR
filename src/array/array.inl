// Array.inl
#ifndef DSTR_ARRAY_IMPLEMENTATION
...
#include "../common/strings/errors.h"
...
namespace dstr {
  ...
  template <typename T>
  template <typename Comparator>
  void Array<T>::compare_sorts(std::ostream& os, Comparator comp) const {
    ...
    long long t_bubble = std::chrono::duration_cast<std::chrono::microseconds>(e1 - s1).count();
    auto s2 = std::chrono::high_resolution_clock::now();
    copy_insert.insertion_sort(comp);
    auto e2 = std::chrono::high_resolution_clock::now();
    long long t_insert = std::chrono::duration_cast<std::chrono::microseconds>(e2 - s2).count();
    auto s3 = std::chrono::high_resolution_clock::now();
    copy_quick.quick_sort(comp);
    auto e3 = std::chrono::high_resolution_clock::now();
    long long t_quick = std::chrono::duration_cast<std::chrono::microseconds>(e3 - s3).count();
    i32 mem = memory_usage();
    os << "\n";
    os << "+--------------------+----------------+----------------+\n";
    os << "| Sort Algorithm     | Time (us)      | Memory (bytes) |\n";
    os << "+--------------------+----------------+----------------+\n";
    os << "| Bubble Sort        | " << std::setw(14) << t_bubble << " | " << std::setw(14) << mem << " |\n";
    os << "| Insertion Sort     | " << std::setw(14) << t_insert << " | " << std::setw(14) << mem << " |\n";
    os << "| Quick Sort         | " << std::setw(14) << t_quick  << " | " << std::setw(14) << mem << " |\n";
    os << "+--------------------+----------------+----------------+\n";
    os << "  Elements: " << size_ << " | Capacity: " << capacity_ << "\n";
  }

  template <typename T>
  template <typename Comparator, typename KeyComp>
  void Array<T>::compare_searches(std::ostream& os, Comparator comp, KeyComp key_comp, const T& target) const {
    i32 r_linear = -1;
    i32 r_binary = -1;
    i32 r_jump   = -1;
    auto s1 = std::chrono::high_resolution_clock::now();
    ...
    auto e1 = std::chrono::high_resolution_clock::now();
    long long t_linear = std::chrono::duration_cast<std::chrono::microseconds>(e1 - s1).count();
    auto s2 = std::chrono::high_resolution_clock::now();
    r_binary = binary_search(comp, key_comp, target);
    auto e2 = std::chrono::high_resolution_clock::now();
    long long t_binary = std::chrono::duration_cast<std::chrono::microseconds>(e2 - s2).count();
    auto s3 = std::chrono::high_resolution_clock::now();
    r_jump = jump_search(comp, key_comp, target);
    auto e3 = std::chrono::high_resolution_clock::now();
    long long t_jump = std::chrono::duration_cast<std::chrono::microseconds>(e3 - s3).count();
    os << "\n";
    os << "+--------------------+------------+--------+\n";
    os << "| Search Algorithm   | Time (us)  | Index  |\n";
    os << "+--------------------+------------+--------+\n";
    os << "| Linear Search      | " << std::setw(10) << t_linear << " | " << std::setw(6) << r_linear << " |\n";
    os << "| Binary Search      | " << std::setw(10) << t_binary << " | " << std::setw(6) << r_binary << " |\n";
    os << "| Jump Search        | " << std::setw(10) << t_jump   << " | " << std::setw(6) << r_jump   << " |\n";
    os << "+--------------------+------------+--------+\n";
    os << "  Elements: " << size_ << "\n";
  }
...
#endif // DSTR_ARRAY_IMPLEMENTATION..
