...
template <typename T>
bool Array<T>::insert(i32 index, const T& value) {
  if (index < 0 || index > size_) {
    return false;
  }
...
void Array<T>::compare_sorts(Comparator comp) const {
  ...
  long long t_bubble =
      std::chrono::duration_cast<std::chrono::microseconds>(end_bubble - start_bubble).count();
  ...
  long long t_quick =
      std::chrono::duration_cast<std::chrono::microseconds>(end_quick - start_quick).count();
...
void Array<T>::compare_searches(Comparator comp, KeyComp key_comp, const T& target) const {
  ...
  long long t_linear =
      std::chrono::duration_cast<std::chrono::microseconds>(end_linear - start_linear).count();
  ...
  long long t_binary =
      std::chrono::duration_cast<std::chrono::microseconds>(end_binary - start_binary).count();
  ...
  long long t_jump =
      std::chrono::duration_cast<std::chrono::microseconds>(end_jump - start_jump).count();
